...
- On Android 12+, font modules will crash apps if you hide Magisk from those apps due to new behavior. Because the fact that hiding Magisk will also unmount all modules files include new and replaced font files. Android 12 and newer have new behavior that font will not be loaded until the app user interface is appeared, modified font files do not exist and app try to read these font files, result in crashing. To overcome this problem, OverlayFS can keep all module font files intact after magisk module files are unmounted for those apps (only if OverlayFS is enabled on modules and hiding OverlayFS is disabled).
